...
def subir_desde_sqlserver(request):
    ...
    try:
        ...
        if not all([user, password, database]):
            messages.error(request, "Configuración de base de datos incompleta en settings.py")
            return redirect('index')
        
        # Crear engine_url para SQL Server con pyodbc
        ...
        request.session['engine_url'] = engine_url
        messages.success(request, f"Conectado exitosamente a SQL Server: {database}")
        return redirect('seleccionar_datos')
        
    except Exception as e:
        messages.error(request, f"Error conectando a SQL Server: {str(e)}")
        return redirect('index')
...
def subir_sql(request):
    ...
    if request.method == 'POST':
        ...
        if archivo_sql:
            ...
            # Función para preparar SQL para reemplazo
            def preparar_sql_para_reemplazo(archivo_sql):
                archivo_sql.seek(0)
                return archivo_sql.read().decode('utf-8', errors='ignore')
...
def seleccionar_datos(request):
    ...
    if request.method == 'POST' and request.POST.get('accion') == 'procesar_columnas' and step == 2:
        ...
        if not tablas_sel:
            ...
            return redirect('seleccionar_datos')        source_type = request.session.get('source_type')
